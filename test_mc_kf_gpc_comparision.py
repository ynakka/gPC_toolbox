...
from gPC_toolbox_v0.initial_conditions import initial_conditions_noinit
from gPC_toolbox_v0.project_gpc_2_statespace import meangpc_noinit
from gPC_toolbox_v0.project_gpc_2_statespace import vargpc_noinit
...
gpc_dyn_dummy = open('lambdified_pendulum_dynamics', 'rb')
gpc_dyn = load(gpc_dyn_dummy)
gpc_dyn_dummy.close()

def f_gpc_dyn(input_vec):
    return np.array(gpc_dyn(*input_vec)).reshape(12)
...
def main():
    ...
    gpc_init = initial_conditions_noinit(initial_condition_mean,num_gpcpoly)

    print('gpc_init',gpc_init)    
    gpc_states = np.zeros((t_steps,num_gpcstates))
    gpc_states[0,:] = gpc_init 
    ...
    for i in range(0,int(t_steps)):
        theta_mean_gpc[i] = meangpc_noinit(gpc_states[i,0:num_gpcpoly],num_gpcpoly)
        theta_dot_mean_gpc[i] = meangpc_noinit(gpc_states[i,num_gpcpoly:int(num_gpcpoly*num_state)],num_gpcpoly)

    # gpc_variance 

    for i in range(0,int(t_steps)):
        theta_var_gpc[i] =  np.sqrt(vargpc_noinit(gpc_states[i,0:num_gpcpoly],num_state,num_uncert,polynomial_degree))
        theta_dot_var_gpc[i] =  np.sqrt(vargpc_noinit(gpc_states[i,num_gpcpoly:int(num_gpcpoly*num_state)],\
    ...
    axs[1].plot(tvec,theta_dot_mean_gpc,label='gPC',color='blue')
    ...
    for trial in range(num_trials):
        axs[0].scatter(tvec,mc_sim[trial][:,0].reshape((t_steps)),s=0.8,color='red',alpha=0.5)
    ...
    axs[1].scatter(tvec,mc_sim[0][:,1].reshape((t_steps)),s=0.5,color='red',alpha=0.5,label='MC')
    for trial in range(1,num_trials):
        axs[1].scatter(tvec,mc_sim[trial][:,1].reshape((t_steps)),s=0.8,color='red',alpha=0.5)
